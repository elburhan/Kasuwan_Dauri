...
class ModuleView(generics.CreateAPIView):
    ...
    def get(self,request):
        ...
        for menu in serialized_menus:
            ...
            menu['fields']['submenu']=Modules.objects.filter(parent_id=menu['pk'],is_active=True,is_menu=True).order_by('display_order').values('id','module_name','module_icon','is_menu','is_active','parent_id','display_order','module_url','module_description')
            cleaned_menus.append(menu['fields'])

        return renderResponse(data=serialized_menus,message='All modules',status=200)